...
bool NyceAxis::GetInSegPars(const double &dDistance,double &dVel, double &dAcc, double &dTime)
{
	const double &dMaxJerk(m_move_pars.dMaxJerk * 2.0 / 3.0);
	const double &dMaxAcc(m_move_pars.dMaxAcc);
	const double &dMaxVel(m_move_pars.dMaxVel);
	double dTime_JerkUniformly(pow(dDistance * 6.0 /dMaxJerk, 1.0 / 3.0 ));//匀加加速时间
	double dTime_Up2MaxAcc((dMaxAcc - dVel) / dMaxJerk);//达到最大加速度时间
